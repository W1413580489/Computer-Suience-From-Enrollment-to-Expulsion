// 新生任务攻略数据 — 基于 guide01.md 全文 + 本地图片
// 4 关串行解锁：登录→主线→支线+道具同时解锁
// 删除补缺篇

export interface GuideSection {
  id: string;
  title: string;
  content: string; // raw markdown with local image paths
}
...
// Image mapping — Vite glob import for build-time hashing
const imageModules = import.meta.glob<{ default: string }>(
  '@/assets/images/guide01/*.{jpg,jpeg,png}',
  { eager: true, query: '?url', import: 'default' }
);

// Build lookup: filename → hashed URL
const img = (name: string): string => {
  for (const [path, url] of Object.entries(imageModules)) {
    if (path.endsWith('/' + name) || path.endsWith(name)) return url as string;
  }
...
const IMG = {
  pig: img('562c89db2002fd42ecc10baf5027d4d4.jpg'),
  deadpool: img('6a5dee60141194371ee6cda667cb9c2e.jpg'),
  memory: img('2be295576003104ef4a1bbfc694eeedb.jpg'),
  kuangkuang: img('image.png'),
  monkey: img('image_1.png'),
  sucai: img('image_2.png'),
  songhao: img('songhao_screenshot.png'),
  genius: img('cc9531ba5fcb1657c36d009373110fad.jpg'),
  score: img('image_3.png'),
  xinke: img('0b915def6e8c03c43f51302509365af7.jpg'),
  campus: img('35351c9d9cb246db924eb30eb341b6d2.jpeg'),
} as const;
    ...
    sections: [
      {
        id: 'login_equip',
        title: '角色装备清单',
        content: `入学前请打开你的「角色背包」，检查以下装备是否齐全：
...
> **JNUID 是最重要的通行证**——它同时是教务系统、校园网、图书馆、选课系统、体测查询等所有校内功能的登录钥匙。`,
      },
      {
        id: 'login_skills',
        title: '技能预加点',
        content: `在正式开学前，建议你学习好以下的一些技能：
...
导航搜索"暨南大学番禺校区"（广州市番禺区兴业大道东855号）。迎新报到当天，社会车辆禁止入校，需在南门下车，换乘校内接驳车。由于东门车道狭窄，容易堵塞，**强烈不建议在东门下车**。`,
      },
      {
        id: 'login_spawn',
        title: '角色重生点',
        content: `番禺校区所有宿舍 24 小时热水，不断水不限电不断电不断网。
    ...
    sections: [
      {
        id: 'main_wall',
        title: '1. 新生四道坎',
        content: `> 新生学习的第一课，就是先把各篇章都看了！想读研，论文文献比这更晦涩；想工作，工作文档比这更复杂。如果你连这点文档都没有耐心阅读，期末周你只会比阅读文档更痛苦。
...
> 如果不想演化成校园自由搏击，请注意沟通。`,
      },
      {
        id: 'main_course',
        title: '2. 新生选课',
        content: `军训开始之后，中间还会遇到选课。由于学生培养方案的不同，学生需要根据本届的需求进行选课。大一新生由于课程固定，只需要在选课网站点击"方案内"，然后选择所有的必修课即可，选择的空间仅有不同老师以及不同的体育课。
...
具体的分数构成老师会在课上公布。`,
      },
      {
        id: 'main_exam',
        title: '4. 新生考试',
        content: `> 本章主要面向理工科。根据不同的时间安排，学校会公布 1~2 周的复习周（注意，新生可能没有复习周！），复习周通常已经结课，由学生自行复习。
...
![宋浩高等数学](${IMG.songhao})`,
      },
      {
        id: 'main_after',
        title: '5. 新生模型后训练',
        content: `考试结束后，你可能会陷入一个贤者状态，开始思考：上课是为了什么？成绩怎么计算？我读大学是为了什么？
...
如果准备提升学历：研究自己的升学方法。`,
      },
      {
        id: 'main_gpa',
        title: '6. 新生绩点综测',
        content: `你可以通过点击网页版教务系统 → 成绩查询 → 全部 → 最好成绩，导出完整成绩单并计算总绩点。
    ...
    sections: [
      {
        id: 'side_title',
        title: '1. 称号与加入势力',
        content: `入学开始，学校各学院会有班委竞选以及学生组织扫楼或者是社团之夜等活动。各位同学可以考虑是否要获取"称号"（班委）或"加入势力"（学生组织/社团）。

称号中，班长、团支书、学委、学代属于特殊称号——前三者对比其他班委有更大概率获得信息差资讯、综测加分、学生干部活动优先、学生干部奖学金等，但是事务繁忙。第四个称号"学生代表"，将自动加入势力"学生代表团"。

![加入信科院势力](${IMG.xinke})

> 本院玩家创立的势力包括：暨大 PTCG 战队、暨大机器人 SSR 战队、无限战队`,
      },
      {
        id: 'side_map',
        title: '2. 大地图探索',
