...
def _chatbot_project() -> Project:
    ...
    rubric_frontend = [
        Rubric(id="rb_frontend_1", task_id="task_frontend", criterion="页面有输入框、消息列表、发送按钮并能点击",
               ...
               required_evidence=["code"], pass_condition="代码含上述元素", visual_check="supported", visual_pass_condition="截图中可见输入框、消息列表与发送按钮三要素", weight=1),
        Rubric(id="rb_frontend_2", task_id="task_frontend", criterion="发送后 fetch 把消息 POST 给后端并显示回答",
               description="前端调用后端 /chat 并渲染返回内容",
               required_evidence=["code", "runtime"], pass_condition="fetch 请求存在且能收到并显示回答", visual_check="supported", visual_pass_condition="截图中同时存在用户发送的消息与机器人返回的回答", weight=1),
    ...
    rubric_review = [
        Rubric(id="rb_review_1", task_id="task_review", criterion="能正常问答：输入问题看到机器人回答",
               description="核心功能可用",
               required_evidence=["runtime"], pass_condition="有运行证据（本地可复现运行说明 / CI 结论 / 部署地址，三选一）", visual_check="supported", visual_pass_condition="截图中同时存在用户输入内容与对应的机器人回复", weight=2),
...
def _agent_project() -> Project:
    ...
    rubric_c2t07 = [
        Rubric(id="rb_c2t07_1", task_id="c2t07", criterion="复杂分析任务中出现 ≥2 次连续工具调用",
               ...
               required_evidence=["code", "trace"], pass_condition="Trace 可见多步", visual_check="supported", visual_pass_condition="终端输出显示同一次任务中出现连续两次及以上工具调用记录", weight=2),
